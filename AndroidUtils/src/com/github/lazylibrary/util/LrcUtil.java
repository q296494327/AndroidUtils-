...
/**
 * 专门用来解析歌词，.lrc;.txt; 使用方法：直接调用getLrcByCurrentPosition()方法
 */
public class LrcUtil {
    ...
    public static Vector<Timelrc> getLrcList() {

	return lrclist;
    }

    public static void setNullLrcList() {
     ...
     * @param f
     */
    public static void ReadLRC(File f) {
	try {
	    if (f == null || !f.exists()) {
		IsLyricExist = false;
		lrclist = null;
	...
	public String toString() {
	    return "Timelrc [lrcString=" + lrcString + ", sleepTime="
		    + sleepTime + ", timePoint=" + timePoint + "]";
	}
    }

    /**
     * 歌词的作用:显示指定位置的歌词 传递一个歌词的进度，返回当前进度歌词
     * 
     * @param current
     *            当前的歌词进度
     */
    public static String RefreshLRC(int current) {
	if (IsLyricExist) {
     ...
     * @return 返回指定的歌词内容
     */
    public static String getLrcByCurrentPosition(String lrcPath,
	    int currentPosition) {
	File lrcFile =getLrcFile(lrcPath);//根据歌曲路径得到lrcFile
	ReadLRC(lrcFile);// 序列化lrc文件
	return RefreshLRC(currentPosition);// 返回指定位置的歌词内容
    }
}
